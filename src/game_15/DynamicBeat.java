...
public class DynamicBeat extends JFrame {
	...
	public DynamicBeat() 
	{
		...
		backButton.setBounds(20, 50, 60, 60); //x위치,y위치,x크기,y크기
		...
		backButton.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseEntered(MouseEvent e) //마우스 커서가 올라갔을때
			{
				backButton.setIcon(backButtonEnteredImage); //이미지 변경
				backButton.setCursor(new Cursor(Cursor.HAND_CURSOR)); //마우스 커서가 hand_cursor로 변경
				Music buttonEnteredMusic = new Music("buttonEnteredMusic.mp3", false);
				buttonEnteredMusic.start();
			}
			@Override
			public void mouseExited(MouseEvent e) //마우스 커서가 아이콘 밖으로 나갔을때
			{
				...
				backButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR)); //마우스 커서가 default_cursor로 다시 변경
			}
			@Override
			public void mousePressed(MouseEvent e) //마우스 클릭을 했을때
			{
	...
	public void screenDraw(Graphics2D g) // double buffering
	{
		...
		{
			g.drawImage(selectedImage, 340, 100, null); //selectedImage를 340x, 100y에 그려줌
	...
	public void gameStart(int nowSelected, String difficulty)
	{
		...
		game.start(); //game 쓰레드 시작.
		
		// 키 이벤트 중요 메소드들: 이유 => https://appdevelopmaster.tistory.com/67
		// KeyEvent가 발생하려면 컴포넌트가 반드시 키보드 포커스를 가지고 있어야 한다. 
		// 키보드 포커스를 얻으려면 requestFocus(), setFocusable(true)을 호출하여야 한다.
		setFocusable(true);
		requestFocus();
		//
	}
